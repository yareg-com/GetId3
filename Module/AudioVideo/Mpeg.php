...
class Mpeg extends BaseHandler
{
    ...
    public function analyze()
    {
        ...
        $MPEGstreamData = fread($this->getid3->fp,
                                min(100000,
        ...
        if ($foundVideo) {
            ...
            if ($info['mpeg']['video']['raw']['intra_quant_flag']) {
                ...
                if ($info['mpeg']['video']['raw']['non_intra_quant_flag']) {
                    $info['mpeg']['video']['raw']['non_intra_quant'] = substr($MPEGstreamData,
                                                                              $VideoChunkOffset,
                                                                              64);
                    $VideoChunkOffset += 64;
                }
        ...
        while (true) {
            ...
            for ($i = 0; $i <= 7; ++$i) {
                ...
                if ($getid3_mp3->decodeMPEGaudioHeader(($AudioChunkOffset + 3) + 8 + $i,
        ...
        if (!empty($info['video']['bitrate']) && !empty($info['audio']['bitrate'])) {
            ...
            $info['playtime_seconds'] *= $this->MPEGsystemNonOverheadPercentage($info['video']['bitrate'],
                                                                                $info['audio']['bitrate']);
